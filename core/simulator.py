...
class Simulator:
    ...
    def run(self, train_mode: bool = False):
        self.scheduler.set_train_mode(train_mode)
        print(f"[DEBUG] Simulator running in {'TRAINING' if train_mode else 'EVALUATION'} mode")
        if train_mode:
        ...
        for r in range(cfg.SIMULATION_ROUNDS):
            ...
            for task in tasks:
                ...
                print(f"[DEBUG] Selected server: {server.id if server else 'None'} for vehicle {task['vehicle_id']}")
                ...
                # Compute transmission and processing
                ...
                if train_mode:
                    # Update scheduler during training
                    self.scheduler.update(base_reward, task, self.mobility)
                else:
                    # Allocate and log metrics during evaluation
                    frag, _ = server.allocate_until_full(
                        task, r,
                        util_threshold=(
                    ...
                    # 1) Grab the exact VE, BS and CL objects used by the scheduler
                    ve        = next(s for s in self.servers if s.id == f"VE_{task['vehicle_id']}")
                    bs_server = next(s for s in self.servers if s.id == f"BS_{bs_idx}")
                    cl_server = next(s for s in self.servers if s.id.startswith("CL_"))

                    # 2) Call calculate_reward with the same signature your scheduler expects
                    ve_cpu = ve.get_cpu_util()
                    ve_mem = ve.get_mem_util()
                    bs_cpu = bs_server.get_cpu_util()
                    bs_mem = bs_server.get_mem_util()

                    rl_reward = self.scheduler.calculate_reward(
                        ve,
                        bs_server,
                        cl_server,
                        server,
                    ...
                    self.reporter.log_and_print(r, task, server, delays, costs, utils, distance, bs_idx, bs_sig, rl_reward)

            if train_mode:
                # Record action distribution per episode
            ...
            # Release completed tasks and update mobility
        ...
        if train_mode:
            ...
            # Export action distribution logs
            ...
            # ✅ Export rewards and epsilon logs
            self.scheduler.save_logs()

        else:
            # Evaluation mode: close reporter and export any scheduler logs
            self.reporter.close()
